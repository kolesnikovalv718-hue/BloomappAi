...
if st.button("Обучить модель"):
    ...
    if not os.path.exists(file_path):
        ...
        if "text" not in df.columns or "bloom" not in df.columns:
            ...
            # График метрик precision/recall/f1-score
            ...
            if strong_classes:
                st.write(f"✅ Сильные классы (хорошо распознаются): {', '.join(strong_classes)}")
            if weak_classes:
                st.write(f"⚠️ Слабые классы (часто путаются): {', '.join(weak_classes)}")
            if recommend_addition:
                st.write("💡 Рекомендуется добавить больше задач для этих классов, чтобы повысить точность и полноту модели")
            if not strong_classes and not weak_classes:
                st.write("Все классы распределены равномерно, дополнительная настройка не требуется")
