...
if page == "Главная":
    ...
    <style>
    body { margin:0; background:#0a0f1f; overflow:hidden; }
    canvas { position:fixed; top:0; left:0; z-index:-1; width:100%; height:100%; }

    .hero { position:relative; text-align:center; padding:60px 20px; color:white; }
    .hero h1 {
        font-size:56px; 
    ...
    .card { background: linear-gradient(135deg, rgba(0,119,255,0.2), rgba(0,180,255,0.1));
    ...
    function draw(){
        ...
        ctx.lineWidth = 2;
        for(let x=0;x<W;x++){
            ctx.lineTo(x,H/2 + 50*Math.sin((x + Date.now()*0.002)/50));
        }
        ctx.stroke();
        ...
        nodes.forEach(n=>{
            ...
            nodes.forEach(o=>{
                ...
                if(dist<100){
                    ...
                    ctx.moveTo(n.x,n.y);
                    ctx.lineTo(o.x,o.y);
                    ctx.strokeStyle='rgba(0,200,255,'+(1-dist/100)*0.3+')';
                    ctx.lineWidth=1;
                    ctx.stroke();
                }
            })
